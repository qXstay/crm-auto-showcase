...
import { updateShiftStaffForBranch } from "@/server/repositories/shift-write-repository";
...
export async function PATCH(
  ...
  try {
    return okJson(
      await updateShiftStaffForBranch({
    ...
    return badRequest(error instanceof Error ? error.message : "Не удалось обновить состав смены.");
  }
}
