...
import { ClientDuplicateError, ClientValidationError, updateClientVehicle } from "@/server/repositories/client-repository";
...
export async function PATCH(
  ...
  try {
    client = await updateClientVehicle(auth.context.currentBranch.id, id, vehicleId, body, {
  ...
  if (!client) {
    return notFound("Автомобиль клиента не найден.");
  }

  await logAuditEvent({
    eventType: "client.vehicle.update",
